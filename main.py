...
def call():
     global filez
     filez = tkinter.filedialog.askdirectory(parent=root,initialdir = "/",title='Choose a file')
     ent1.delete(0,"end") 
     ent1.insert(0, filez)
     ...
     for filename in dirs:
         ...
         c = tk.Checkbutton(sf.frame, text=filename, variable=intvar_dict[filename])
         c.pack()
...
def copy_file():
    for key, value in intvar_dict.items():
        if value.get() > 0:
            src_path = filez +"//"+key
            if (os.path.isfile(src_path)) == True: # Does bob.txt exist?  Is it a file, or a directory?
                shutil.copy(src_path,destination) 
            elif (os.path.isdir(src_path)) == True:
                shutil.copytree(src_path,destination+"//"+key,symlinks=False,ignore=None)
            else:
                pass    
